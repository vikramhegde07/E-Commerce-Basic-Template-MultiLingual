...
export default function ProductsPageContent() {
    ...
    const pushQuery = React.useCallback(
        (overrides: Partial<Record<string, string | number | undefined>> = {}, replace = false) => {
            const query = {
                ...
                category_slug: (category === 'all' ? '' : category) || undefined, // send empty to browse all
    ...
    React.useEffect(() => {
        ...
        setCategory(typeof q.category_slug === "string" ? q.category_slug : "");
        setOrderBy(typeof q.orderBy === "string" ? q.orderBy : "name-asc");
        setPage(typeof q.page === "string" ? Number(q.page) || 1 : 1);
        setPerPage(typeof q.perPage === "string" ? Number(q.perPage) || 15 : 15);
        setView(q.view === "list" ? "list" : "grid");
    ...
    React.useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                if (category) params.category_slug = (category === 'all' ? '' : category); // your backend will accept category slug (e.g., "doors")
    ...
    const handleCategory = (v: string) => {
        ...
        setCategory(next);
        setPage(1);
        pushQuery({ category_slug: v === "all" ? "" : v, page: 1 });
    };
    const handleOrderBy = (v: string) => {
        setOrderBy(v);
        setPage(1);
        pushQuery({ orderBy: v, page: 1 });
    };
    const handlePerPage = (v: string) => {
        const n = Number(v) || 15;
        setPerPage(n);
        setPage(1);
        pushQuery({ perPage: n, page: 1 });
    };

    const setGridView = () => {
        setView("grid");
        pushQuery({ view: undefined }); // will be dropped
    };
